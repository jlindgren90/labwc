...
static lab_data_buffer_ptr rounded_rect(struct rounded_corner_ctx *ctx);
...
load_button(struct button *b, int active)
{
	 ...
	 */
	if (!img.valid() && (b->state_set & LAB_BS_HOVERD)) {
		img = button_imgs[b->type][b->state_set & ~LAB_BS_HOVERD];
...
theme_builtin(void)
{
	...
	g_theme.osd_window_switcher_width = 600;
	g_theme.osd_window_switcher_width_is_percent = false;
	g_theme.osd_window_switcher_padding = 4;
	g_theme.osd_window_switcher_item_padding_x = 10;
	g_theme.osd_window_switcher_item_padding_y = 1;
	g_theme.osd_window_switcher_item_active_border_width = 2;
	g_theme.osd_window_switcher_item_icon_size = -1;
...
entry(const char *key, const char *value)
{
	...
	if (match_glob(key, "osd.window-switcher.width")) {
		if (strrchr(value, '%')) {
			g_theme.osd_window_switcher_width_is_percent = true;
		} else {
			g_theme.osd_window_switcher_width_is_percent = false;
		}
		g_theme.osd_window_switcher_width =
			get_int_if_positive(value, "osd.window-switcher.width");
	}
	if (match_glob(key, "osd.window-switcher.padding")) {
		g_theme.osd_window_switcher_padding = get_int_if_positive(value,
	...
	if (match_glob(key, "osd.window-switcher.item.padding.y")) {
		g_theme.osd_window_switcher_item_padding_y =
			get_int_if_positive(value,
...
post_processing(void)
{
	...
	if (g_theme.osd_window_switcher_item_icon_size <= 0) {
		g_theme.osd_window_switcher_item_icon_size = osd_font_height;
	}
	int osd_field_height = MAX(osd_font_height,
		g_theme.osd_window_switcher_item_icon_size);
	g_theme.osd_window_switcher_item_height =
		osd_field_height
		+ 2 * g_theme.osd_window_switcher_item_padding_y
	...
	if (g_theme.osd_window_switcher_width_is_percent) {
		g_theme.osd_window_switcher_width =
			MIN(g_theme.osd_window_switcher_width, 100);
	}
...
theme_finish(void)
{
	...
	for (int active = THEME_INACTIVE; active <= THEME_ACTIVE; active++) {
		...
		g_theme.window[active].titlebar_fill = {};
		g_theme.window[active].corner_top_left_normal = {};
		g_theme.window[active].corner_top_right_normal = {};
		g_theme.window[active].shadow_corner_top = {};
		g_theme.window[active].shadow_corner_bottom = {};
