...
static void
update_icon(struct view *view)
{
	if (!view->xwayland_surface) {
		return;
	}

	xcb_window_t window_id = view->xwayland_surface->window_id;
...
handle_map(struct wl_listener *listener, void *data)
{
	...
	if (!view->st->ever_mapped) {
		view_set_initial_geom(view->id, /* rel_to */ NULL,
	 ...
	 * for handle_commit().
	 */
	view_commit_geom(view->id, view->st->pending.width,
...
xwayland_view_from_window_id(xcb_window_t id)
{
	for (int i = view_count() - 1; i >= 0; i--) {
		...
		if (view->xwayland_surface
				&& view->xwayland_surface->window_id == id) {
			return view;
		}
	}
	return NULL;
