...
{
	set_cairo_color(cairo, g_theme.window_button_hover_bg_color);
...
static void
load_button(struct button *b, enum ssd_active_state active)
{
...
load_buttons(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(buttons); ++i) {
		...
		load_button(b, SSD_INACTIVE);
		load_button(b, SSD_ACTIVE);
	}
}
...
theme_builtin(void)
{
	...
	parse_hexstr("#aaaaaa", g_theme.window[SSD_ACTIVE].border_color);
	parse_hexstr("#aaaaaa", g_theme.window[SSD_INACTIVE].border_color);

	parse_hexstr("#ff0000", g_theme.window_toggled_keybinds_color);

	g_theme.window[SSD_ACTIVE].title_bg.gradient = LAB_GRADIENT_NONE;
	g_theme.window[SSD_INACTIVE].title_bg.gradient = LAB_GRADIENT_NONE;
	parse_hexstr("#e1dedb", g_theme.window[SSD_ACTIVE].title_bg.color);
	parse_hexstr("#f6f5f4", g_theme.window[SSD_INACTIVE].title_bg.color);
	g_theme.window[SSD_ACTIVE].title_bg.color_split_to[0] = FLT_MIN;
	g_theme.window[SSD_INACTIVE].title_bg.color_split_to[0] = FLT_MIN;
	g_theme.window[SSD_ACTIVE].title_bg.color_to[0] = FLT_MIN;
	g_theme.window[SSD_INACTIVE].title_bg.color_to[0] = FLT_MIN;
	g_theme.window[SSD_ACTIVE].title_bg.color_to_split_to[0] = FLT_MIN;
	g_theme.window[SSD_INACTIVE].title_bg.color_to_split_to[0] = FLT_MIN;

	parse_hexstr("#000000", g_theme.window[SSD_ACTIVE].label_text_color);
	parse_hexstr("#000000", g_theme.window[SSD_INACTIVE].label_text_color);
	...
	parse_hexstr("#80808020", g_theme.window_button_hover_bg_color);
	...
	g_theme.osd_window_switcher_classic.item_active_border_color[0] = FLT_MIN;
...
entry(const char *key, const char *value)
{
	...
	if (match_glob(key, "border.width")) {
		g_theme.border_width = get_int_if_positive(
			value, "border.width");
	}
	if (match_glob(key, "window.titlebar.padding.width")) {
		g_theme.window_titlebar_padding_width = get_int_if_positive(
			value, "window.titlebar.padding.width");
	}
	if (match_glob(key, "window.titlebar.padding.height")) {
		g_theme.window_titlebar_padding_height = get_int_if_positive(
	...
	if (match_glob(key, "window.active.border.color")) {
		parse_color(value, g_theme.window[SSD_ACTIVE].border_color);
	}
	if (match_glob(key, "window.inactive.border.color")) {
		parse_color(value, g_theme.window[SSD_INACTIVE].border_color);
	}
	...
	if (match_glob(key, "window.active.title.bg")) {
		g_theme.window[SSD_ACTIVE].title_bg.gradient = parse_gradient(value);
	}
	if (match_glob(key, "window.inactive.title.bg")) {
		g_theme.window[SSD_INACTIVE].title_bg.gradient = parse_gradient(value);
	}
	if (match_glob(key, "window.active.title.bg.color")) {
		parse_color(value, g_theme.window[SSD_ACTIVE].title_bg.color);
	}
	if (match_glob(key, "window.inactive.title.bg.color")) {
		parse_color(value, g_theme.window[SSD_INACTIVE].title_bg.color);
	}
	if (match_glob(key, "window.active.title.bg.color.splitTo")) {
		parse_color(value, g_theme.window[SSD_ACTIVE].title_bg.color_split_to);
	}
	if (match_glob(key, "window.inactive.title.bg.color.splitTo")) {
		parse_color(value, g_theme.window[SSD_INACTIVE].title_bg.color_split_to);
	}
	if (match_glob(key, "window.active.title.bg.colorTo")) {
		parse_color(value, g_theme.window[SSD_ACTIVE].title_bg.color_to);
	}
	if (match_glob(key, "window.inactive.title.bg.colorTo")) {
		parse_color(value, g_theme.window[SSD_INACTIVE].title_bg.color_to);
	}
	if (match_glob(key, "window.active.title.bg.colorTo.splitTo")) {
		parse_color(value, g_theme.window[SSD_ACTIVE].title_bg.color_to_split_to);
	}
	if (match_glob(key, "window.inactive.title.bg.colorTo.splitTo")) {
		parse_color(value, g_theme.window[SSD_INACTIVE].title_bg.color_to_split_to);
	}

	if (match_glob(key, "window.active.label.text.color")) {
		parse_color(value, g_theme.window[SSD_ACTIVE].label_text_color);
	}
	if (match_glob(key, "window.inactive.label.text.color")) {
		parse_color(value, g_theme.window[SSD_INACTIVE].label_text_color);
	}
	...
	if (match_glob(key, "window.button.spacing")) {
		g_theme.window_button_spacing = get_int_if_positive(
			value, "window.button.spacing");
	}

	/* botton hover overlay */
	if (match_glob(key, "window.button.hover.bg.color")) {
		parse_color(value, g_theme.window_button_hover_bg_color);
	}
	if (match_glob(key, "window.button.hover.bg.corner-radius")) {
		g_theme.window_button_hover_bg_corner_radius = get_int_if_positive(
	...
	if (match_glob(key, "menu.width.min")) {
		g_theme.menu_min_width = get_int_if_positive(
			value, "menu.width.min");
	}
	if (match_glob(key, "menu.width.max")) {
		g_theme.menu_max_width = get_int_if_positive(
			value, "menu.width.max");
	}
	if (match_glob(key, "menu.border.width")) {
		g_theme.menu_border_width = get_int_if_positive(
	...
	if (match_glob(key, "menu.items.padding.x")) {
		g_theme.menu_items_padding_x = get_int_if_positive(
			value, "menu.items.padding.x");
	}
	if (match_glob(key, "menu.items.padding.y")) {
		g_theme.menu_items_padding_y = get_int_if_positive(
	...
	if (match_glob(key, "menu.separator.width")) {
		g_theme.menu_separator_line_thickness = get_int_if_positive(
			value, "menu.separator.width");
	}
	if (match_glob(key, "menu.separator.padding.width")) {
		g_theme.menu_separator_padding_width = get_int_if_positive(
			value, "menu.separator.padding.width");
	}
	if (match_glob(key, "menu.separator.padding.height")) {
		g_theme.menu_separator_padding_height = get_int_if_positive(
	...
	if (match_glob(key, "osd.border.width")) {
		g_theme.osd_border_width = get_int_if_positive(
	...
	if (match_glob(key, "snapping.overlay.region.border.width")) {
		g_theme.snapping_overlay_region.border_width = get_int_if_positive(
			value, "snapping.overlay.region.border.width");
	}
	if (match_glob(key, "snapping.overlay.edge.border.width")) {
		g_theme.snapping_overlay_edge.border_width = get_int_if_positive(
...
create_backgrounds(void)
{
	...
	FOR_EACH_ACTIVE_STATE(active) {
		g_theme.window[active].titlebar_pattern = create_titlebar_pattern(
...
post_processing(void)
{
	 ...
	 */
	int icon_padding = g_theme.window_button_width / 10;
	g_theme.window_icon_size =
		MIN(g_theme.window_button_width - 2 * icon_padding,
			g_theme.window_button_height);

	fill_background_colors(&g_theme.window[SSD_INACTIVE].title_bg);
	fill_background_colors(&g_theme.window[SSD_ACTIVE].title_bg);
	...
	int osd_field_height = MAX(osd_font_height, g_theme.window_icon_size);
	...
	if (g_theme.window_button_hover_bg_corner_radius > min_button_hover_radius) {
		g_theme.window_button_hover_bg_corner_radius = min_button_hover_radius;
	}
	...
	if (g_theme.osd_bg_color[0] == FLT_MIN) {
		memcpy(g_theme.osd_bg_color,
			g_theme.window[SSD_ACTIVE].title_bg.color,
	...
	if (g_theme.osd_label_text_color[0] == FLT_MIN) {
		memcpy(g_theme.osd_label_text_color,
			g_theme.window[SSD_ACTIVE].label_text_color,
	...
	if (g_theme.snapping_overlay_region.border_color[0][0] == FLT_MIN) {
		fill_colors_with_osd_theme(g_theme.snapping_overlay_region.border_color);
	}
	if (g_theme.snapping_overlay_edge.border_color[0][0] == FLT_MIN) {
		fill_colors_with_osd_theme(g_theme.snapping_overlay_edge.border_color);
	}
}
...
theme_finish(void)
{
	for (enum lab_node_type type = LAB_NODE_BUTTON_FIRST;
		...
		for (uint8_t state_set = LAB_BS_DEFAULT;
			...
			destroy_img(&g_theme.window[SSD_INACTIVE]
				.button_imgs[type][state_set]);
			destroy_img(&g_theme.window[SSD_ACTIVE]
				.button_imgs[type][state_set]);
		}
	}
