...
{
	const char *app_id = view_get_string_prop(m_view, "app_id");
	const char *wlr_app_id = m_handle->app_id;
	if (app_id && wlr_app_id && !strcmp(app_id, wlr_app_id)) {
		/* Don't send app_id if they are the same */
		return;
	}
	wlr_foreign_toplevel_handle_v1_set_app_id(m_handle, app_id);
}

void
wlr_foreign_toplevel::handle_new_title(void *)
{
	const char *title = view_get_string_prop(m_view, "title");
	const char *wlr_title = m_handle->title;
	if (title && wlr_title && !strcmp(title, wlr_title)) {
		/* Don't send title if they are the same */
		return;
	}
	wlr_foreign_toplevel_handle_v1_set_title(m_handle, title);
}
...
wlr_foreign_toplevel::handle_new_outputs(void *)
{
	...
	for (auto output : g_server.outputs) {
		if (view_on_output(m_view, output.get())) {
