...
{
	assert(g_seat.overlay.active.edge != VIEW_EDGE_INVALID
...
static bool
edge_has_adjacent_output_from_cursor(struct output *output, enum view_edge edge)
{
	return wlr_output_layout_adjacent_output(g_server.output_layout,
		get_wlr_direction(edge), output->wlr_output, g_seat.cursor->x,
		g_seat.cursor->y);
}

static void
show_edge_overlay(enum view_edge edge, struct output *output)
{
...
overlay_update(void)
{
	...
	enum view_edge edge = edge_from_cursor(&output);
