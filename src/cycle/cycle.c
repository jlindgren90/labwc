...
cycle_on_cursor_release(struct wlr_scene_node *node)
{
	...
	server.cycle.current_idx = item->cycle_idx;
...
cycle_begin(enum lab_cycle_dir direction)
{
	...
	if (direction == LAB_CYCLE_DIR_FORWARD) {
		...
		if (len > 1 && active && cycle_list_nth(0) == active->id) {
			server.cycle.current_idx = 1;
		} else {
			server.cycle.current_idx = 0;
		}
	} else {
		server.cycle.current_idx = len - 1;
	}
...
cycle_step(enum lab_cycle_dir direction)
{
	...
	if (direction == LAB_CYCLE_DIR_FORWARD) {
		server.cycle.current_idx =
			(server.cycle.current_idx + 1) % len;
	} else {
		server.cycle.current_idx =
			(server.cycle.current_idx + len - 1) % len;
	}
...
cycle_finish(bool switch_focus)
{
	...
	ViewId selected_id = cycle_list_nth(server.cycle.current_idx);
	destroy_cycle();
