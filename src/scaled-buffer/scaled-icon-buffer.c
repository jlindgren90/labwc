...
{
	struct lab_img *img = desktop_entry_load_icon(view->server,
		view->icon.name, icon_size, scale);
	if (img) {
...
{
	struct lab_img *img = desktop_entry_load_icon_from_app_id(view->server,
...
{
#if HAVE_LIBSFDO
	float scale = view->server->max_output_scale;
	...
	img = desktop_entry_load_icon(view->server,
