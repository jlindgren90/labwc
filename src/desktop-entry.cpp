...
desktop_entry_init(void)
{
	...
	if (!sfdo->basedir_ctx) {
		wlr_log(WLR_ERROR, "Failed to initialize basedir ctx");
		return;
	}

	sfdo->desktop_ctx = sfdo_desktop_ctx_create(sfdo->basedir_ctx);
	if (!sfdo->desktop_ctx) {
		wlr_log(WLR_ERROR, "Failed to initialize desktop ctx");
		return;
	}

	sfdo->icon_ctx = sfdo_icon_ctx_create(sfdo->basedir_ctx);
	if (!sfdo->icon_ctx) {
		wlr_log(WLR_ERROR, "Failed to initialize icon ctx");
		return;
	}
	...
	if (!sfdo->desktop_db) {
		wlr_log(WLR_ERROR, "Failed to initialize desktop db");
		return;
	}
	...
	if (!sfdo->icon_theme) {
		wlr_log(WLR_ERROR, "Failed to initialize icon theme");
		return;
	}
