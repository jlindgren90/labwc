...
bool
xdg_toplevel_view::contains_window_type(window_type window_type)
{
	auto toplevel = xdg_toplevel_from_view(this);
	struct wlr_xdg_toplevel_state *state = &toplevel->current;
...
void
xdg_toplevel_view::handle_commit(void *)
{
	auto view = this;
...
{
	uint32_t serial =
		wlr_xdg_toplevel_set_maximized(xdg_toplevel_from_view(this),
			maximized == VIEW_AXIS_BOTH);
	if (serial > 0) {
		set_pending_configure_serial(this, serial);
	}
}
...
xdg_toplevel_view::get_root()
{
	...
	auto surface = (wlr_xdg_surface *)root->base;
...
xdg_toplevel_view::get_children()
{
	...
	for (auto view : g_views.reversed()) {
		...
		if (top_parent_of(view.get()) != toplevel) {
			continue;
		}
...
{
	uint32_t serial =
		wlr_xdg_toplevel_set_activated(xdg_toplevel_from_view(this),
			activated);
	if (serial > 0) {
		set_pending_configure_serial(this, serial);
	}
...
{
	uint32_t serial =
		wlr_xdg_toplevel_set_fullscreen(xdg_toplevel_from_view(this),
			fullscreen);
	if (serial > 0) {
		set_pending_configure_serial(this, serial);
	}
...
xdg_toplevel_view::notify_tiled()
{
	...
	if (want_edge) {
		switch (view->tiled) {
		case VIEW_EDGE_LEFT:
			edge = (wlr_edges)(WLR_EDGE_LEFT | WLR_EDGE_TOP
				| WLR_EDGE_BOTTOM);
			break;
		case VIEW_EDGE_RIGHT:
			edge = (wlr_edges)(WLR_EDGE_RIGHT | WLR_EDGE_TOP
				| WLR_EDGE_BOTTOM);
			break;
		case VIEW_EDGE_UP:
			edge = (wlr_edges)(WLR_EDGE_TOP | WLR_EDGE_LEFT
				| WLR_EDGE_RIGHT);
			break;
		case VIEW_EDGE_DOWN:
			edge = (wlr_edges)(WLR_EDGE_BOTTOM | WLR_EDGE_LEFT
				| WLR_EDGE_RIGHT);
			break;
		default:
			edge = WLR_EDGE_NONE;
	...
	if (want_region && view->tiled_region) {
		...
		edge = (wlr_edges)(WLR_EDGE_LEFT | WLR_EDGE_RIGHT
...
static struct view *
lookup_view_by_xdg_toplevel(struct wlr_xdg_toplevel *xdg_toplevel)
{
	for (auto view : g_views) {
		if (view->type != LAB_XDG_SHELL_VIEW) {
			continue;
		}
		if (xdg_toplevel_from_view(view.get()) == xdg_toplevel) {
			return view.get();
		}
	}
	return NULL;
...
set_initial_position(struct view *view)
{
	...
	if (parent_xdg_toplevel) {
		...
		struct view *parent =
			lookup_view_by_xdg_toplevel(parent_xdg_toplevel);
		assert(parent);
...
const char *
xdg_toplevel_view::get_string_prop(const char *prop)
{
	auto xdg_toplevel = xdg_surface->toplevel;
	if (!xdg_toplevel) {
		/*
...
void
xdg_toplevel_view::unmap(bool client_request)
{
	auto view = this;
	if (view->mapped) {
		view->mapped = false;
