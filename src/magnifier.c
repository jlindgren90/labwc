...
magnifier_draw(struct output *output, struct wlr_buffer *output_buffer, struct wlr_box *damage)
{
	...
	if (!tmp_buffer) {
		tmp_buffer = wlr_allocator_create_buffer(g_server.allocator,
			mag_box.width, mag_box.height,
	...
	if (!tmp_texture) {
		tmp_texture =
			wlr_texture_from_buffer(g_server.renderer, tmp_buffer);
	}
	if (!tmp_texture) {
	...
	struct wlr_render_pass *tmp_render_pass =
		wlr_renderer_begin_buffer_pass(g_server.renderer, tmp_buffer,
			NULL);
	if (!tmp_render_pass) {
	...
	struct wlr_texture *output_texture =
		wlr_texture_from_buffer(g_server.renderer, output_buffer);
	if (!output_texture) {
		goto cleanup;
	}
...
{ /* !goto */
	...
	tmp_render_pass = wlr_renderer_begin_buffer_pass(g_server.renderer,
		output_buffer, NULL);
	if (!tmp_render_pass) {
...
enable_magnifier(bool enable)
{
	...
	g_server.scene->WLR_PRIVATE.direct_scanout =
		enable ? false : g_server.direct_scanout_enabled;
}
