	...
	float *bg_color)
{
	...
	/* Create background */
	lab_wlr_scene_rect_create(tree, bg_width, g_theme.menu_item_height, bg_color);
...
title_create_scene(struct menuitem *menuitem, int *item_y)
{
	...
	lab_wlr_scene_rect_create(menuitem->normal_tree,
		bg_width, g_theme.menu_header_height, bg_color);
...
menu_on_view_destroy(ViewId view_id)
{
	...
	if (g_server.menu_current
			&& g_server.menu_current->triggered_by_view == view_id) {
		menu_close_root();
	}
}
