	...
	float *bg_color)
{
	...
	int label_max_width =
		bg_width - 2 * theme->menu_items_padding_x - arrow_width;
...
init_windowmenu(struct server *server)
{
	...
	if (!menu) {
		...
		item = item_create(menu, _("Decorations"), false);
...
menu_execute_item(struct menuitem *item)
{
	 ...
	 */
	actions_run(item->parent->triggered_by_view, server, &item->actions, NULL);
