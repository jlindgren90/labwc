...
handle_sigchld(int signal, void *data)
{
	...
	if (g_server.xwayland_server
			&& info.si_pid == g_server.xwayland_server->pid) {
		return 0;
	}
...
server_global_filter(const struct wl_client *client, const struct wl_global *global, void *data)
{
	...
	struct wl_client *xwayland_client = g_server.xwayland_server
		? g_server.xwayland_server->client
		: NULL;
...
server_init(void)
{
	...
	g_server.view_tree = lab_wlr_scene_tree_create(&g_server.scene->tree);
