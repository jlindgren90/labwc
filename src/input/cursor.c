...
update_pressed_surface(const struct cursor_context *ctx)
{
	...
	if (g_seat.pressed.ctx.surface
			&& ctx->surface != g_seat.pressed.ctx.surface) {
		...
		struct cursor_context *notified_ctx)
{
	...
	if (g_seat.pressed.ctx.surface
			&& ctx->surface != g_seat.pressed.ctx.surface
			&& !update_pressed_surface(ctx) && !g_seat.drag.active) {
		if (notified_ctx) {
			/*
...
{
	double lx, ly;
	wlr_cursor_absolute_to_layout_coords(g_seat.cursor, device, x, y, &lx, &ly);
