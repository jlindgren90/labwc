...
new_keyboard(struct wlr_input_device *device, bool is_virtual)
{
	...
	wlr_seat_set_keyboard(g_seat.wlr_seat, kb);
...
seat_init(void)
{
	g_seat.wlr_seat = wlr_seat_create(server.wl_display, "seat0");
	if (!g_seat.wlr_seat) {
	...
	CONNECT_SIGNAL(server.backend, &g_seat, new_input);
	CONNECT_SIGNAL(&g_seat.wlr_seat->keyboard_state, &g_seat, focus_change);

	g_seat.virtual_pointer = wlr_virtual_pointer_manager_v1_create(
		server.wl_display);
	CONNECT_SIGNAL(g_seat.virtual_pointer, &g_seat, new_virtual_pointer);

	g_seat.virtual_keyboard = wlr_virtual_keyboard_manager_v1_create(
		server.wl_display);
	...
	wlr_cursor_attach_output_layout(g_seat.cursor, server.output_layout);
...
{
	if (!surface || !wlr_seat_pointer_has_grab(g_seat.wlr_seat)) {
		return;
	}
...
seat_reconfigure(void)
{
	...
	wl_list_for_each(input, &g_seat.inputs, link) {
		...
		case WLR_INPUT_DEVICE_TOUCH:
			...
			break;
		case WLR_INPUT_DEVICE_TABLET:
			map_input_to_output(input->wlr_input_device, rc.tablet.output_name);
			break;
		default:
			break;
		}
...
void
seat_force_focus_surface(struct wlr_surface *surface)
{
		...
		bool replace_exclusive_layer, bool is_lock_surface)
{
	...
	if (!surface) {
		wlr_seat_keyboard_notify_clear_focus(g_seat.wlr_seat);
	...
	if (!wlr_seat_get_keyboard(g_seat.wlr_seat)) {
		 ...
		 */
		wlr_seat_set_keyboard(g_seat.wlr_seat, &g_seat.keyboard_group->keyboard);
	}
	...
	enum lab_cursors cursor_shape)
{
	...
	g_seat.focus_override.surface = g_seat.wlr_seat->keyboard_state.focused_surface;
