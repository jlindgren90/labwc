...
static void set_surface(struct xwayland_view *view,
...
{ /* !goto */
	...
	for (; iter.rem; xcb_ewmh_get_wm_icon_next(&iter)) {
		...
		std::vector<uint8_t> buf(iter.height * stride);
...
static void
set_surface(struct xwayland_view *view, struct wlr_surface *surface)
{
	if (view->surface) {
		/* Disconnect wlr_surface event listeners */
		view->on_commit.disconnect();
		view->on_surface_destroy.disconnect();
	}
	view->surface = surface;
	if (surface) {
		/* Connect wlr_surface event listeners */
		view->on_commit.connect(&surface->events.commit);
		view->on_surface_destroy.connect(&surface->events.destroy);
	}
}
