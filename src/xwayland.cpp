...
bool
xwayland_view::contains_window_type(window_type window_type)
{
...
xwayland_view::get_children()
{
	...
	for (auto view : g_views.reversed()) {
		...
		if (top_parent_of(view.get()) != xwayland_surface) {
			continue;
		}
...
xwayland_view_from_window_id(xcb_window_t id)
{
	for (auto view : g_views) {
		...
		auto xwayland_view = xwayland_view_from_view(view.get());
