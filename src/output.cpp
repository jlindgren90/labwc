...
output_apply_gamma(struct output *output)
{
	...
	struct wlr_gamma_control_v1 *gamma_control =
		wlr_gamma_control_manager_v1_get_control(
			g_server.gamma_control_manager_v1, output->wlr_output);
...
add_output_to_layout(struct output *output)
{
	...
	if (g_server.session_lock_manager->locked) {
		session_lock_output_create(g_server.session_lock_manager,
			output);
	}
}
...
handle_new_output(struct wl_listener *listener, void *data)
{
	...
	output->layer_popup_tree = wlr_scene_tree_create(&g_server.scene->tree);
	node_descriptor_create(&output->layer_popup_tree->node,
		LAB_NODE_DESC_TREE, NULL);
	output->osd_tree = wlr_scene_tree_create(&g_server.scene->tree);
	node_descriptor_create(&output->osd_tree->node,
		LAB_NODE_DESC_TREE, NULL);
	output->session_lock_tree =
		wlr_scene_tree_create(&g_server.scene->tree);
...
output_init(void)
{
	...
	wl_signal_add(&g_server.backend->events.new_output,
		&g_server.new_output);
...
output_config_apply(struct wlr_output_configuration_v1 *config)
{
	...
	wl_list_for_each(head, &config->heads, link) {
		...
		if (output_enabled) {
			...
			wlr_output_layout_get_box(g_server.output_layout, o,
				&pos);
...
{
	g_server.output_manager =
		wlr_output_manager_v1_create(g_server.wl_display);
	...
	g_server.gamma_control_set_gamma.notify =
		handle_gamma_control_set_gamma;
...
output_nearest_to(int lx, int ly)
{
	...
	return output_from_wlr_output(wlr_output_layout_output_at(
		g_server.output_layout, closest_x, closest_y));
}

struct output *
output_nearest_to_cursor(void)
{
	return output_nearest_to(g_seat.cursor->x, g_seat.cursor->y);
}
...
output_update_all_usable_areas(bool layout_changed)
{
	...
	for (auto output : g_server.outputs) {
		if (update_usable_area(output.get())) {
			usable_area_changed = true;
			regions_update_geometry(output.get());
