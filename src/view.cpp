...
view_matches_query(struct view *view, struct view_query *query)
{
	...
	if (query->window_type != LAB_WINDOW_TYPE_INVALID
			&& !view->contains_window_type(query->window_type)) {
		return false;
	}
	...
	const char *tiled_region = nullptr;
	if (CHECK_PTR(view->tiled_region, region)) {
		tiled_region = region->name.c();
	}
...
view_iter
view_find_matching(view_iter start, lab_view_criteria criteria)
{
	for (; start; ++start) {
...
{
	view_list matching;
	for_each_view(view, g_views.begin(), criteria) {
		matching.append(view.get());
	}
	return matching;
}
...
view_minimize(struct view *view, bool minimized)
{
	 ...
	 */
	struct view *root = view->get_root();
...
view_cascade(struct view *view)
{
	...
	while (candidate_updated) {
		...
		for_each_view(other_view, g_views.begin(),
				LAB_VIEW_CRITERIA_CURRENT_WORKSPACE) {
			auto other = ssd_max_extents(other_view.get());
			if (other_view.get() == view || view->minimized
...
view_apply_region_geometry(struct view *view)
{
	...
	if (view->tiled_region_evacuate) {
		...
		view->tiled_region.reset(regions_from_name(
			view->tiled_region_evacuate.c(), output).get());

		/* Get rid of the evacuate instruction */
		view->tiled_region_evacuate.clear();
	...
	struct wlr_box geo =
		view_get_region_snap_box(view, view->tiled_region.get());
...
{
	assert(view);
	CHECK_PTR_OR_RET(view->tiled_region, region);
	if (!view->tiled_region_evacuate) {
		view->tiled_region_evacuate = region->name;
	}
	view->tiled_region.reset();
}
		...
		bool across_outputs, bool combine, bool store_natural_geometry)
{
	...
	if (lab_edge_is_cardinal(edge) && view->maximized == VIEW_AXIS_NONE
		...
		auto parallel_mask = lab_edge(edge | invert_edge);
		/*
		 ...
		 */
		auto parallel_tiled = lab_edge(view->tiled & parallel_mask);
		auto orthogonal_tiled = lab_edge(view->tiled & ~parallel_mask);
		...
		if (across_outputs && view->tiled == edge) {
			 ...
			 */
			edge = lab_edge(view->tiled & ~parallel_mask);
...
view_move_to_output(struct view *view, struct output *output)
{
	...
	if (view_is_floating(view)) {
		...
		view_apply_tiled_geometry(view);
	} else if (CHECK_PTR(view->tiled_region, old_region)) {
		auto region = regions_from_name(old_region->name.c(), output);
...
{
	auto iter = lab::find_ptr(g_views.begin(), view);
	assert(iter);
...
view_set_surface(struct view *view, struct wlr_surface *surface)
{
	...
	if (surface) {
		CONNECT_LISTENER(surface, view, map);
		CONNECT_LISTENER(surface, view, unmap);
		CONNECT_LISTENER(surface, view, commit);
	}
}
...
view_update_visibility(struct view *view)
{
	...
	if (visible) {
		 ...
		 * focus (so g_server.active_view remains NULL). To avoid
...
view::view(view_type type, struct workspace &workspace)
