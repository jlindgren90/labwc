...
get_next_cycle_view(struct view *start_view, enum lab_cycle_dir dir)
{
	...
	auto end = forwards ? g_views.end() : g_views.rend();
	auto pos = start_view ? std::find(begin, end, start_view) : end;
	 ...
	 */
	if (pos == end && forwards) {
		pos = view_find_matching(begin, end, criteria); // top view
	}
	if (pos != end) {
		pos = view_find_matching(++pos, end, criteria); // next view
	}
	if (pos == end) {
		pos = view_find_matching(begin, end, criteria); // wrap around
	}
	return (pos != end) ? pos.get() : nullptr;
}
	...
	for (auto view : views) {
		...
		for (auto &field : rc.window_switcher.fields) {
			...
			if (field.content == LAB_FIELD_ICON) {
				...
				buf_clear(&buf);
				osd_field_get_content(&field, &buf, view.get());
...
update_osd(void)
{
	...
	if (views.begin() == views.end() || !g_server.osd_state.cycle_view) {
		osd_finish();
		return;
	}
