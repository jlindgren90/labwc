...
struct session_lock_output : public destroyable,
		public refcounted<session_lock_output>
{
	...
	::wlr_surface *wlr_surface() {
		return surface ? surface->surface->surface : nullptr;
	}
...
lock_output_reconfigure(struct session_lock_output *output)
{
	...
	if (output->surface) {
		wlr_session_lock_surface_v1_configure(output->surface->surface,
...
{
	for (auto lock_output : manager->lock_outputs) {
		auto tree = lock_output->tree;
		lock_output.reset(); // release ref
...
session_lock_manager::handle_new_lock(void *data)
{
	...
	for (auto output : g_server.outputs) {
		session_lock_output_create(manager, output.get());
	}
...
session_lock_update_for_layout_change(void)
{
	...
	for (auto output : g_server.outputs) {
		align_session_lock_tree(output.get());
	}

	struct session_lock_manager *manager = g_server.session_lock_manager;
	for (auto lock_output : manager->lock_outputs) {
		lock_output_reconfigure(lock_output.get());
	}
}
