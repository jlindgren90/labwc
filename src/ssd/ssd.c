...
struct ssd *
ssd_create(const struct ssd_create_params *params)
{
	struct ssd *ssd = znew(*ssd);

	ssd->view_id = params->view_id;
	ssd->tree = lab_wlr_scene_tree_create(params->scene_tree);

	/*
	 * Attach node_descriptor to the root node so that get_cursor_context()
	 * detect cursor hovering on borders and extents.
	 */
	node_descriptor_create(&ssd->tree->node, LAB_NODE_SSD_ROOT,
		params->view_id, /*data*/ NULL);
	 ...
	 * anymore.
	 */
	ssd_titlebar_create(ssd, params->state, params->icon_buffer);
	ssd_border_create(ssd, params->state);
	ssd_set_active(ssd, params->state->active);
	ssd->state.geometry = params->state->current;
...
ssd_destroy(struct ssd *ssd)
{
	...
	if (server.hovered_button && node_view_from_node(
			server.hovered_button->node) == ssd->view_id) {
