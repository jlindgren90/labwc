...
ssd_titlebar_create(struct ssd *ssd)
{
	...
	FOR_EACH_ACTIVE_STATE(active) {
		...
		for (int b = 0; b < rc.num_title_buttons_left; b++) {
		...
		for (int b = rc.num_title_buttons_right - 1; b >= 0; b--) {
			x -= theme->window_button_width + theme->window_button_spacing;
...
update_visible_buttons(struct ssd *ssd)
{
	...
	int button_count_left = rc.num_title_buttons_left;
	int button_count_right = rc.num_title_buttons_right;
