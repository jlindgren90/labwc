...
ssd_titlebar_create(struct ssd *ssd)
{
	...
	FOR_EACH_STATE(ssd, subtree) {
		...
		struct ssd_part *bg_part =
			add_scene_part(&subtree->parts, LAB_NODE_TITLEBAR);
		...
		for (auto b : rc.title_buttons_left) {
			auto imgs = g_theme.window[active].button_imgs[b];
			add_scene_button(&subtree->parts, b, parent, imgs, x, y,
				view);
		...
		x = width - g_theme.window_titlebar_padding_width
			+ g_theme.window_button_spacing;
		for (auto b : rc.title_buttons_right) { // note: right-to-left
			x -= g_theme.window_button_width
				+ g_theme.window_button_spacing;
			auto imgs = g_theme.window[active].button_imgs[b];
			add_scene_button(&subtree->parts, b, parent, imgs, x, y,
...
set_squared_corners(struct ssd *ssd, bool enable)
{
	...
	FOR_EACH_STATE(ssd, subtree) {
		part = ssd_get_part(&subtree->parts, LAB_NODE_TITLEBAR);
		...
		part = ssd_get_part(&subtree->parts, LAB_NODE_TITLEBAR_CORNER_LEFT);
		wlr_scene_node_set_enabled(part->node, !enable);

		part = ssd_get_part(&subtree->parts, LAB_NODE_TITLEBAR_CORNER_RIGHT);
		wlr_scene_node_set_enabled(part->node, !enable);

		/* (Un)round the corner buttons */
		if (!rc.title_buttons_left.empty()) {
			auto button_type = rc.title_buttons_left.front();
			part = ssd_get_part(&subtree->parts, button_type);
			struct ssd_button *button = node_ssd_button_from_node(part->node);
			update_button_state(button, LAB_BS_ROUNDED, !enable);
		}
		if (!rc.title_buttons_right.empty()) { // note: right-to-left
			auto button_type = rc.title_buttons_right.front();
			part = ssd_get_part(&subtree->parts, button_type);
			struct ssd_button *button = node_ssd_button_from_node(part->node);
			update_button_state(button, LAB_BS_ROUNDED, !enable);
		}
...
update_visible_buttons(struct ssd *ssd)
{
	...
	FOR_EACH_STATE(ssd, subtree) {
		button_count = 0;
		for (auto b : rc.title_buttons_left) {
			part = ssd_get_part(&subtree->parts, b);
...
ssd_titlebar_update(struct ssd *ssd)
{
	...
	FOR_EACH_STATE(ssd, subtree) {
		...
		part = ssd_get_part(&subtree->parts, LAB_NODE_TITLEBAR_CORNER_RIGHT);
		wlr_scene_node_set_position(part->node, x,
			-g_theme.border_width);
...
ssd_update_title_positions(struct ssd *ssd, int offset_left, int offset_right)
{
	...
	struct ssd_sub_tree *subtree;
	FOR_EACH_STATE(ssd, subtree) {
		part = ssd_get_part(&subtree->parts, LAB_NODE_TITLE);
		if (!part || !part->node) {
		...
		x = offset_left;
		y = (g_theme.titlebar_height - buffer_height) / 2;
...
get_title_offsets(struct ssd *ssd, int *offset_left, int *offset_right)
{
	...
	*offset_right = padding_width;
	...
	for (auto b : rc.title_buttons_left) {
		struct ssd_part *part = ssd_get_part(&subtree->parts, b);
...
ssd_update_title(struct ssd *ssd)
{
	...
	FOR_EACH_STATE(ssd, subtree) {
		...
		text_color = g_theme.window[active].label_text_color;
...
ssd_update_button_hover(struct wlr_scene_node *node)
{
	...
	/* Disable old hover */
	if (g_ssd_hover_state.button) {
		update_button_state(g_ssd_hover_state.button, LAB_BS_HOVERED, false);
		g_ssd_hover_state.view = NULL;
		g_ssd_hover_state.button = NULL;
	}
	if (button) {
		update_button_state(button, LAB_BS_HOVERED, true);
		g_ssd_hover_state.view = button->view;
		g_ssd_hover_state.button = button;
	}
}
