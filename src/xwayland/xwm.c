...
#define IS_UNMANAGED(xsurface) (!(xsurface)->info->view_id)
...
static struct xwayland_surface *xwayland_surface_create(
	...
	surface->info = xsurface_add(window_id, surface);
...
bool
xwayland_window_is_focused(xcb_window_t window_id)
{
	struct lab_xwm *xwm = g_xserver.xwm;
	return xwm && xwm->focused_window == window_id;
}
...
static void read_surface_class(struct lab_xwm *xwm,
	...
	view_set_app_id(surface->info->view_id, instance);
	free(instance);
}
...
static void read_surface_title(struct lab_xwm *xwm,
	...
	view_set_title(xsurface->info->view_id, title);
	free(title);
}
...
static void read_surface_parent(struct lab_xwm *xwm,
	...
	if (reply->type != XCB_ATOM_NONE && xid != NULL) {
		xsurface_set_parent_xid(xsurface->window_id, *xid);
	} else {
		xsurface_set_parent_xid(xsurface->window_id, XCB_NONE);
	}
}
...
static void read_surface_motif_hints(struct lab_xwm *xwm,
		struct xwayland_surface *xsurface,
		xcb_get_property_reply_t *reply) {
	if (reply->value_len == 0) {
		view_enable_ssd(xsurface->info->view_id, true);
		return;
	}
	...
	if (motif_hints[MWM_HINTS_FLAGS_FIELD] & MWM_HINTS_DECORATIONS) {
		...
		view_enable_ssd(xsurface->info->view_id, decorated);
	}
}

static void
read_surface_strut_partial(struct lab_xwm *xwm,
		struct xwayland_surface *xsurface,
		xcb_get_property_reply_t *reply)
{
	if (reply->type == XCB_ATOM_NONE) {
		view_set_strut_partial(xsurface->info->view_id, NULL);
		return;
	}
	...
	view_set_strut_partial(xsurface->info->view_id, &strut_partial);
}
...
static void read_surface_net_wm_state(struct lab_xwm *xwm,
	...
	view_fullscreen(xsurface->info->view_id, fullscreen, /* output */ NULL);
	view_maximize(xsurface->info->view_id, maximized);
	view_minimize(xsurface->info->view_id, minimized);
	view_set_always_on_top(xsurface->info->view_id, always_on_top);
}
...
static void lab_xwm_handle_net_wm_moveresize_message(struct lab_xwm *xwm,
		xcb_client_message_event_t *ev) {
	struct xwayland_surface *xsurface = xsurface_lookup(ev->window);
	if (!xsurface) {
		return;
...
static void lab_xwm_handle_net_wm_state_message(struct lab_xwm *xwm,
	...
	const ViewState *state = view_get_state(xsurface->info->view_id);
...
static void lab_xwm_handle_net_close_window_message(struct lab_xwm *xwm,
	...
	struct xwayland_surface *surface = xsurface_lookup(ev->window);
	if (surface) {
		view_close(surface->info->view_id);
	}
}
...
static void lab_xwm_handle_wm_change_state_message(struct lab_xwm *xwm,
	...
	view_minimize(xsurface->info->view_id, minimize);
}
...
static void lab_xwm_handle_focus_in(struct lab_xwm *xwm,
	...
	if (xsurface) {
		if (!IS_UNMANAGED(xsurface)) {
			xwm->focused_window = xsurface->window_id;
		}
		xsurface_focus(xsurface->window_id, xsurface->surface,
			/* raise */ false);
	}
