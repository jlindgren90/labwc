...
static int lab_xwm_selection_flush_source_data(
	...
	lab_xwm_schedule_flush(transfer->selection->xwm);
...
static int lab_xwm_data_source_read(int fd, uint32_t mask, void *data) {
	...
	if (transfer->source_data.size >= INCR_CHUNK_SIZE) {
		if (!transfer->incr) {
			...
			lab_xwm_selection_transfer_remove_event_source(transfer);
			lab_xwm_selection_send_notify(xwm, &transfer->request, true);
			...
			lab_xwm_selection_flush_source_data(transfer);
		}
	} else if (len == 0 && !transfer->incr) {
		wlr_log(WLR_DEBUG, "non-incr transfer complete");
		lab_xwm_selection_flush_source_data(transfer);
		lab_xwm_selection_send_notify(xwm, &transfer->request, true);
		lab_xwm_selection_transfer_destroy_outgoing(transfer);
	} else if (len == 0 && transfer->incr) {
...
error_out:
	lab_xwm_selection_send_notify(xwm, &transfer->request, false);
	lab_xwm_selection_transfer_destroy_outgoing(transfer);
	return 0;
}
...
void lab_xwm_send_incr_chunk(struct lab_xwm_selection_transfer *transfer) {
	...
	if (transfer->flush_property_on_delete) {
		...
		int length = lab_xwm_selection_flush_source_data(transfer);

		if (transfer->wl_client_fd >= 0) {
			lab_xwm_selection_transfer_start_outgoing(transfer);
		} else if (length > 0) {
 ...
 */
static bool lab_xwm_selection_send_data(struct lab_xwm_selection *selection,
	...
	struct lab_xwm_selection_transfer *transfer = calloc(1, sizeof(*transfer));
	if (transfer == NULL) {
		wlr_log(WLR_ERROR, "Allocation failed");
		return false;
	}

	lab_xwm_selection_transfer_init(transfer, selection);
	...
	wlr_log(WLR_DEBUG, "Sending Wayland selection %u to Xwayland window with "
		...
		req->target, transfer);
	lab_xwm_selection_source_send(selection, mime_type, p[1]);
...
static void lab_xwm_selection_send_targets(struct lab_xwm_selection *selection,
	...
	wl_array_for_each(mime_type_ptr, mime_types) {
		...
		targets[2+i] = lab_xwm_mime_type_to_atom(xwm, mime_type);
		++i;
	}
...
void lab_xwm_handle_selection_request(struct lab_xwm *xwm,
	...
	struct lab_xwm_selection *selection =
		lab_xwm_get_selection(xwm, req->selection);
	if (selection == NULL) {
	...
	if (xwm->focus_surface == NULL && !dnd_allowed) {
		if (wlr_log_get_verbosity() >= WLR_DEBUG) {
			char *selection_name = lab_xwm_get_atom_name(xwm, selection->atom);
	...
	if (req->target == xwm->atoms[TARGETS]) {
		lab_xwm_selection_send_targets(selection, req);
		...
		bool send_success = lab_xwm_selection_send_data(selection, req, mime_type);
		free(mime_type);
		if (!send_success) {
			goto fail_notify_requestor;
		}
