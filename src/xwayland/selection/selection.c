...
void xwm_selection_transfer_init(struct xwm_selection_transfer *transfer,
	...
	*transfer = (struct xwm_selection_transfer){
		.selection = selection,
		.wl_client_fd = -1,
	};
}
...
static int xwm_handle_selection_property_notify(struct xwm *xwm,
	...
	for (size_t i = 0; i < sizeof(selections)/sizeof(selections[0]); ++i) {
		...
		struct xwm_selection_transfer *outgoing;
...
int xwm_handle_selection_event(struct xwm *xwm,
...
void xwm_selection_init(struct xwm_selection *selection,
	...
	*selection = (struct xwm_selection){
		.xwm = xwm,
		.atom = atom,
...
void xwm_selection_finish(struct xwm_selection *selection) {
	...
	struct xwm_selection_transfer *incoming;
...
static void xwm_selection_set_owner(struct xwm_selection *selection,
		bool set) {
	if (set) {
...
static void seat_handle_start_drag(struct wl_listener *listener, void *data) {
	struct xwm *xwm = wl_container_of(listener, xwm, seat_start_drag);
