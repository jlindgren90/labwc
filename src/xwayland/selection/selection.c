...
void lab_xwm_selection_transfer_init(struct lab_xwm_selection_transfer *transfer,
	...
	*transfer = (struct lab_xwm_selection_transfer){
		.selection = selection,
		.wl_client_fd = -1,
	};
}
...
void lab_xwm_selection_transfer_destroy(
	...
	lab_xwm_selection_transfer_destroy_property_reply(transfer);
	lab_xwm_selection_transfer_remove_event_source(transfer);
...
char *lab_xwm_mime_type_from_atom(struct lab_xwm *xwm, xcb_atom_t atom) {
	if (atom == xwm->atoms[UTF8_STRING]) {
		...
		return lab_xwm_get_atom_name(xwm, atom);
	}
}
...
static int lab_xwm_handle_selection_property_notify(struct lab_xwm *xwm,
		xcb_property_notify_event_t *event) {
	struct lab_xwm_selection *selections[] = {
	...
	for (size_t i = 0; i < sizeof(selections)/sizeof(selections[0]); ++i) {
		struct lab_xwm_selection *selection = selections[i];

		if (event->state == XCB_PROPERTY_NEW_VALUE &&
				event->atom == xwm->atoms[WL_SELECTION]) {
			struct lab_xwm_selection_transfer *transfer =
				lab_xwm_selection_find_incoming_transfer_by_window(selection,
						event->window);
			if (transfer) {
				if (transfer->incr) {
					lab_xwm_get_incr_chunk(transfer);
				}
		...
		struct lab_xwm_selection_transfer *outgoing;
		wl_list_for_each(outgoing, &selection->outgoing, link) {
			if (event->window == outgoing->request.requestor) {
				if (event->state == XCB_PROPERTY_DELETE &&
						...
						outgoing->incr) {
					lab_xwm_send_incr_chunk(outgoing);
				}
				return 1;
			}
		}
...
int lab_xwm_handle_selection_event(struct lab_xwm *xwm,
	...
	case XCB_SELECTION_NOTIFY:
		lab_xwm_handle_selection_notify(xwm, (xcb_selection_notify_event_t *)event);
		return 1;
	case XCB_PROPERTY_NOTIFY:
		return lab_xwm_handle_selection_property_notify(xwm,
			(xcb_property_notify_event_t *)event);
	case XCB_SELECTION_REQUEST:
		lab_xwm_handle_selection_request(xwm,
	...
	case XCB_XFIXES_SELECTION_NOTIFY:
		...
		return lab_xwm_handle_xfixes_selection_notify(xwm,
...
void lab_xwm_selection_finish(struct lab_xwm_selection *selection) {
	...
	struct lab_xwm_selection_transfer *outgoing, *tmp;
	...
	struct lab_xwm_selection_transfer *incoming;
...
static void handle_seat_set_selection(struct wl_listener *listener,
	...
	lab_xwm_selection_set_owner(&xwm->clipboard_selection, source != NULL);
}
...
static void handle_seat_set_primary_selection(struct wl_listener *listener,
	...
	lab_xwm_selection_set_owner(&xwm->primary_selection, source != NULL);
}

static void seat_handle_start_drag(struct wl_listener *listener, void *data) {
	struct lab_xwm *xwm = wl_container_of(listener, xwm, seat_start_drag);
	struct wlr_drag *drag = data;

	lab_xwm_selection_set_owner(&xwm->dnd_selection, drag != NULL);
