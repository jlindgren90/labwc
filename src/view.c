		...
		enum lab_edge edge)
{
	...
	if (view) {
		struct border margin = ssd_get_margin(view->st);
		dst.x += margin.left;
		dst.y += margin.top;
...
_minimize(struct view *view, bool minimized, bool *need_refocus)
{
	 ...
	 */
	*need_refocus |=
		(minimized ? (view == server.active_view) : view->st->mapped);
}
...
view_apply_natural_geometry(struct view *view)
{
	...
	view_ensure_geom_onscreen(view->st, &geometry);
...
view_apply_maximized_geometry(struct view *view)
{
	...
	if (view->st->maximized != VIEW_AXIS_BOTH) {
		view_ensure_geom_onscreen(view->st, &natural);
	}
