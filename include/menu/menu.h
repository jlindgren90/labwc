...
struct menuitem : public ref_owned<menuitem>, public weak_target<menuitem> {
...
struct menu : public ref_owned<menu>, public weak_target<menu> {
	lab_str id;
	lab_str label;
	lab_str icon_name;
	lab_str execute;
	weakptr<menu> parent;
	refptr<menu_pipe_context> pipe_ctx;

	struct {
		int width;
		int height;
	} size;
	reflist<menuitem> menuitems;
	struct {
		weakptr<::menu> menu;
