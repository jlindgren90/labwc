...
struct view : public destroyable, public refcounted<view>, public view_data {
	...
	virtual bool contains_window_type(window_type window_type) = 0;
 ...
 * @v: Name of view pointer.
 * @range: Range to iterate over (note: evaluated twice).
 ...
 *	}
 */
#define for_each_view(v, range, criteria)                                    \
	auto _v_iter = (range).begin();                                      \
	auto _v_end = (range).end();                                         \
	for (refptr<struct view> v;                                          \
		(_v_iter = view_find_matching(_v_iter, _v_end, (criteria)))  \
			!= _v_end && (v = *_v_iter);                         \
		++_v_iter)
 ...
 * @stop: End of range to iterate over.
 * @criteria: Criteria to match against.
 *
 * Returns @stop if there are no views matching the criteria.
 */
view_iter view_find_matching(view_iter start, view_iter stop,
	lab_view_criteria criteria);
