...
struct view_impl {
	...
	bool (*has_panel_strut)(struct view *self);
};
...
/**
 * view_has_panel_strut() - returns true for panel-like views that reserve
 * space at a screen edge. These views are treated as if they have the
 * fixedPosition window rule applied: i.e. they are not restricted to
